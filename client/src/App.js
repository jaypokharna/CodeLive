...
const App = () => {
  ...
  const runCode = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:5000/runcode', { message, inputData });
  ...
  const handleSubmit = async () => {
    if (roomId && password) {
      try {
        const response = await axios.post('http://localhost:5000/join-room', { roomId, password });
